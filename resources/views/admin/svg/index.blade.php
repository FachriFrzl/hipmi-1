@extends('layouts.app', ['title' => 'svg - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-900 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.svg.create') }}" class="text-white flex"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
TAMBAH</a>
            ...
            <div class="relative mx-4">
                ...
                <form action="{{ route('admin.svg.index') }}" method="GET">
                        ...
                        @forelse($svg as $item)
                            <tr class="border bg-white">
                                <td class="px-10 py-2 flex justify-center">
                                    <img src="{{asset('storage/svg/'.$item->image)}}" class="admin-index-image">
                                </td>
        
                                <td class="px-5 py-2">
                                    <b>{{ $item->nama }}</b>
                                </td>
                                <td class="px-5 py-2">{{$item -> deskripsi}}
                                </td>
                                <td class="px-10 py-2 text-center">
                                    <div class="flex">
                                        <a href="{{route('admin.svg.show',$item->id)}}" class="bg-indigo-600 px-2 py-2 mx-2 rounded shadow-sm text-xs text-white focus:outline-none"><span class="material-symbols-outlined">
                                            info
                                            </span>
                                        </a>
